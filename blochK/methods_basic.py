...
def sample_BZ(Lq):
    """Samples the square Brillouin zone, i.e. square with edges (-pi,-pi),(-pi,pi),(pi,pi),(pi,-pi)"""
    epsilon = pi/Lq #ensures that the edge is not sampled twice
    ks = np.meshgrid(np.linspace(-pi+epsilon,pi-epsilon,Lq),np.linspace(-pi+epsilon,pi-epsilon,Lq),indexing='ij')
    return np.array(ks)


def sample_reducedBZ(Lq):
    """Samples the reduced Brillouin zone, i.e. square with edges (0,pi),(pi,0),(0,-pi),(-pi,0)"""
    epsilon = pi/Lq #ensures that the edge is not sampled twice
    ks = np.meshgrid(np.linspace(-pi+epsilon,pi-epsilon,Lq),np.linspace(-pi+epsilon,pi-epsilon,Lq),indexing='ij')
